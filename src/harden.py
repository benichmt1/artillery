...
if is_posix():
    ...
    if os.path.isfile("/etc/ssh/sshd_config"):
        ...
        if match:
			warning = warning + "[!] Issue identified: Password authentication enabled. An attacker may be able to brute force weak passwords.\n\r\n\r"
        
		match = re.search("Protocol 1|Protocol 2,1", data)
        #
        if match:
            #triggered
            warning = warning + "[!] Issue identified: SSH Protocol 1 enabled which is potentially vulnerable to MiTM attacks. https://www.kb.cert.org/vuls/id/684820\n\r\n\r"
    #
    # check ftp config
    #
